...
//            boolean isValidCommand = handleDELETE(splitClientMessage);
//            return isValidCommand;
//        } else {
    ...
    public static boolean checkValidityPUT(String[] splitClientMessage) {
        if (splitClientMessage.length != 3) {
            ...
            if (isAlphaNum && isInt) { // valid input
                ...
                System.out.println("Unsuccessful operation: PUT's key must contain only alphanumeric characters and PUT's value must be a non-negative integer");
                return false;
            } else if (!isAlphaNum) {
                System.out.println("Unsuccessful operation: PUT's key must contain only alphanumeric characters");
                return false;
            } else {
                System.out.println("Unsuccessful operation: PUT's value must be a non-negative integer");
                return false;
            }
        }
    }

    public synchronized boolean checkValidityGET(String[] splitClientMessage) {
        if (splitClientMessage.length != 2) {
            ...
            boolean isAlphaNum = isAlphaNumeric(splitClientMessage[1]);
            if (!isAlphaNum) {
                System.out.println("Unsuccessful operation: GET's key must contain only alphanumeric characters");
                return false;
            }
        }
        return true;
